...
lemma monadic_rewrite_symb_exec_l_known_nF:
  ...
  by (erule monadic_rewrite_trans[rotated])
     (rule monadic_rewrite_symb_exec_l_nF[OF monadic_rewrite_pre_imp_refl]; simp)
...
lemma monadic_rewrite_symb_exec_r_known_nE:
  ...
  apply (rule monadic_rewrite_guard_imp)
   apply (erule monadic_rewrite_trans)
   apply (rule monadic_rewrite_symb_exec_r_nE[OF monadic_rewrite_pre_imp_refl, rotated -1], simp+)
  done
...
lemma monadic_rewrite_symb_exec_l_drop_F:
  "\<lbrakk> monadic_rewrite True E P g h; \<And>P. \<lbrace>P\<rbrace> m \<lbrace>\<lambda>_. P\<rbrace>; empty_fail m \<rbrakk>
...
lemma monadic_rewrite_symb_exec_l_drop_nF:
  "\<lbrakk> monadic_rewrite False E P g h; \<And>P. \<lbrace>P\<rbrace> m \<lbrace>\<lambda>_. P\<rbrace>; empty_fail m; no_fail P' m \<rbrakk>
...
lemma monadic_rewrite_symb_exec_r_drop_E:
  "\<lbrakk> monadic_rewrite F True P g h; \<And>P. \<lbrace>P\<rbrace> m \<lbrace>\<lambda>_. P\<rbrace>; empty_fail m; no_fail P' m \<rbrakk>
...
lemma monadic_rewrite_symb_exec_r_drop_nE:
  "\<lbrakk> monadic_rewrite F False P g h; \<And>P. \<lbrace>P\<rbrace> m \<lbrace>\<lambda>_. P\<rbrace>; no_fail P' m \<rbrakk>
...
(*
The most common way of performing monadic rewrite is by doing a pass over the LHS/RHS via some
